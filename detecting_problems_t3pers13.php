<?php
    ...
    while($row_list = $list->fetch(PDO::FETCH_ASSOC)):
        $src = $row_list["domain"];
        $sql_query_domain = "select ipv4 as ipv4 from serverlookup where domain = '" . $src . "';";
    ...
    while($row_list_two = $list_two->fetch(PDO::FETCH_ASSOC)):
        ...
        $sql_query_domain_two = "select ipv6 as ipv6 from serverlookup where domain = '" . $src_two . "';";
        ...
        endwhile;
        $sql_query_dest_two = "select DISTINCT dest as dest from traceroute where src = '" . $six_two . "';";
